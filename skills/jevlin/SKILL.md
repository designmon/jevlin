...
**If `grep`/`rg` can answer it, use grep.** It is faster, exact, and never wrong. `jevlin` earns
its turn only where no pattern expresses the question.

## What it is measured to do — and not do

Benchmarked on 24 real commits across two repos (`~/jevlin/SPIKE-RESULTS.md`), against a
...
> by reading. Never delete, refactor or report based on a `jevlin` list alone.

## Commands

```bash
# narrow candidates: one per line on stdin, the ones that qualify on stdout
git ls-files | jevlin filter "relevant to adding Stripe webhook handling" --top 10
rg -n "TODO" src | jevlin filter "a real blocker, not a nice-to-have" --min 0.7
git log --oneline -50 | jevlin filter "likely to have broken the build" --rank --scores

# yes/no over a blob: THE EXIT CODE IS THE ANSWER
npm run build 2>&1 | tail -100 | jevlin ask "the build succeeded with no type errors" && echo ok
git diff | jevlin ask "this changes a public API or an on-disk format" --min 0.5

# anything else: pass {state, questions} straight through
echo '{"state":{...},"questions":{"q":{"type":"noul","instructions":"..."}}}' | jevlin raw
```

Give candidates context with a TAB — everything after it is sent to jevlin and never echoed:
...
| **3** | **DEGRADED — jevlin never saw this** (no key, timeout, HTTP, daily cap) | **do the task the slow way** |
...
operating on a truncated world. **1 means jevlin thought about it and said no; 3 means it never
...
distribution before trusting one. If stderr says `flat distribution`, jevlin did not understand
the question: rephrase it or add `--context`.

Receipts go to stderr, results to stdout, so piping stays clean. `jevlin stats` shows what it has
