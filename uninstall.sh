...
rm -f "$HOME/.claude/skills/jev"
rm -f "$HOME/.codex/skills/jev"

echo "stripping the jev hooks from ~/.claude/settings.json…"
"$N" -e '
...
if(s.env) delete s.env.JEV_WATCH;
s.permissions.allow=(s.permissions.allow??[]).filter(r=>r!=="Bash(jev *)");
fs.writeFileSync(p, JSON.stringify(s,null,2)+"\n");
console.log("  settings.json cleaned");
'
echo
echo "done. ~/jev and ~/.local/state/jev/*.jsonl were left in place."
